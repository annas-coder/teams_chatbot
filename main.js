    ...
    constructor() {
        ...
        this.onMessage(async (context, next) => {
            ...
            if (text === 'submit timesheet' || text === 'timesheet') {
                ...
                await context.sendActivity('Type "submit timesheet" to log your hours for the day.');
            } else {
                await context.sendActivity('Welcome to the Attendance Bot! Type "submit timesheet" to begin, or "help" for options.');
            }
            await next();
        });
...
function buildCardBody(projectData) {
    ...
    const totalHours = projectData.reduce((sum, project) => sum + (parseFloat(project.hours) || 0), 0);
    body.push({
        "type": "TextBlock",
    ...
    try {
        ...
        for (const project of projectData) {
            ...
            if (!projectId || isNaN(hours) || !tasksCompleted) {
                await context.sendActivity('❌ Error: Please fill in all required fields for all projects (Project, Hours, Tasks).');
                return;
            }
            ...
            await promisePool.query(
                `INSERT INTO submissions (employee_id, project_id, date, hours, task_type, tasks, remarks, timestamp) 
                 VALUES (?, ?, ?, ?, ?, ?, ?, NOW())`,
                [userId, projectId, submissionDate, hours, taskType, tasksCompleted, remarks]
            );
        }

        // Send success message
        if (totalHours < 8) {
            await context.sendActivity(`⚠ Warning: Total hours (${totalHours}) less than 8! Submission was still recorded.`);
        } else {
            await context.sendActivity(`✅ Timesheet submitted successfully! Total hours: ${totalHours}`);
        }

    } catch (error) {
        console.error('Database insertion error:', error);
        await context.sendActivity('❌ Sorry, there was an error saving your timesheet. Please try again.');
    }
}
...
cron.schedule('0 18 * * *', async () => {
    ...
    try {
        const [employees] = await promisePool.query('SELECT teams_id, name FROM employees');
            ...
            if (conversationReference) {
                try {
                    await adapter.continueConversation(
                        ...
                        async (turnContext) => {
                            await turnContext.sendActivity(`Hi ${emp.name}! 👋 Don't forget to submit your timesheet for today.`);
                            const card = createTimesheetCard();
                    ...
                    console.error(`Failed to send proactive message to ${emp.name}:`, proactiveError);
                }
            } else {
                console.log(`No conversation reference found for ${emp.name}. They need to message the bot first.`);
            }
        }
    } catch (dbError) {
